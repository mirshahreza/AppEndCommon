...
namespace AppEnd
{
    public static class StaticMethods
    {
        ...
        {
            if (s == null || s.Trim() == "") ThrowEx($"The parameter [{paramName}] can not be null or empty");
        }

        public static void ValidateIfExist(string path)
        {
            if (File.Exists(path)) ThrowEx($"The file [{path}] already exist.");
        }

        public static void ValidateIfNotExist(string path)
        {
            if (!File.Exists(path)) ThrowEx($"The file [{path}] does not exist.");
        }
        ...
        public static IEnumerable<string> GetFiles(string path, string? searchPattern = null)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                try
                {
                    files = searchPattern.IsNullOrEmpty() ? Directory.GetFiles(path) : Directory.GetFiles(path, searchPattern);
                }
                catch (Exception ex)
                {
                    Console.Error.WriteLine(ex);
        ...
        public static string GetCacheKeyForFiles(string filePath)
        {
        ...
        public static byte[] ResizeImage(byte[] imageFile, int targetSize)
        {
