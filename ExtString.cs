...
namespace AppEndCommon
{
    public static partial class ExtString
    {
        ...
        {
            s = s.Replace(RepeatN(NL, 4), NL);
            s = s.Replace(RepeatN(NL, 3), NL);
            s = s.Replace(RepeatN(NL, 2), NL);
            return s;
        }

        public static void ValidateStringNotNullOrEmpty(this string s, string paramName)
        {
            if (s == null || s.Trim() == "") new ExtException($"CanNotBeNullOrEmpty", System.Reflection.MethodBase.GetCurrentMethod())
