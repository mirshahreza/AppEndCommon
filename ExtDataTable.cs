...
namespace AppEndCommon
{
	public static class ExtDataTable
	{
		public static void ToCSV(this DataTable dt, string tempDirPath, List<string>? exceptColumns = null, Dictionary<string, string>? columnTitles = null)
		{
			...
			{
				if (columnTitles is not null && columnTitles.TryGetValue(dt.Columns[i].ColumnName, out string? value)) sw.Write(value.Replace(",", "_").Replace(ExtString.NL, ""));
			...
			foreach (DataRow dr in dt.Rows)
			{
				for (int i = 0; i < dt.Columns.Count; i++)
				{
					if (!Convert.IsDBNull(dr[i]))
					{
						...
						if (value.Contains(',')) sw.Write(String.Format("\"{0}\"", value.Replace(ExtString.NL, "")));
						else sw.Write(value.Replace(ExtString.NL, ""));
					}
					if (i < dt.Columns.Count - 1) sw.Write(",");
				}
				sw.Write(sw.NewLine);
